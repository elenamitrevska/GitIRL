...
<div class="more-from-category">
	...
	foreach ( $categories as $category ) {
		...
		echo '<div class="category-container">';
			echo '<div class="top">';
				echo '<span class="section-title">' . esc_html_x( 'Повеќе од', 'Повеќе од POST CATEGORY', 'mission-news' ) . ' <span>' . esc_html( get_cat_name( $category ) ) . '</span></span>';
			// Translators: %s is the name of the post category
			echo '<a class="category-link" href="' . esc_url( get_category_link( $category ) ) . '">' . sprintf( esc_html__( 'Се од %s', 'mission-news' ), esc_html( get_cat_name( $category ) ) ) . ' &raquo;</a>';
			echo '</div>';
			echo '<ul>';
